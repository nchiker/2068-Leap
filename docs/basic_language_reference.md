...
| `FLASH <n>` / `BRIGHT <n>` / `OVER <n>` / `INVERSE <n>` | Text attributes — `FLASH`/`BRIGHT`/`INVERSE` implemented (0/1, affect subsequent `PRINT` output the same way `INK`/`PAPER` do; `INVERSE 1` swaps the current ink/paper at print time without changing the underlying `INK`/`PAPER` values; `BRIGHT` sets the attribute byte's bit 6, which existed and was always writable but had no statement setting it until now). `OVER <n>` stores its 0/1 state (for static-checker validation and forward compatibility) but doesn't yet change how text is plotted — real `OVER 1` XOR-plots onto the existing bitmap instead of overwriting it, which needs a new `kernel/graphics` putchar variant not built yet |
