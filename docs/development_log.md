# Development log: build order, module-by-module testing, and diagnostics

This is the detailed, module-by-module build history that used to live in
`README.md` directly — recommended build order, per-module test procedures
(`kernel/memory`, `kernel/math`, the calculator engine, `basic/`, the
editor, graphics, I/O, storage), and the one-off diagnostic ROMs built
along the way. Moved here so `README.md` can stay focused on running the
finished product; nothing below changed, only where it lives.

For the current, condensed feature list and status, see `README.md`. For
full design rationale and the complete bug-by-bug history, see
[`docs/programmers_reference.md`](programmers_reference.md).
