...
; ============================================================================
; GFX_CPLOT
; Coarse 2x2-per-cell block-graphics plot — CPLOT's mechanism. Fills a
; single 4x4-pixel quadrant of one character cell by OR/XOR-ing a
; repeated byte pattern into 4 consecutive scanline bytes, rather than
; four individual GFX_WRITE_PIXEL-style pixel writes — verified in
; Python that this byte-level shortcut produces exactly the same
; result as plotting all 16 pixels individually, across random
; pre-existing cell content (2000 trials, zero mismatches), before any
; Z80 was written. No new font glyphs needed — this writes directly to
; the bitmap, entirely separate from FONT_TABLE/GFX_CHAR_TO_FONT_
; OFFSET's character-code machinery (which is for text, not this).
;
; Coarse coordinates: cx is 0-63 (256 pixels / 4), cy is 0-47 (192
; pixels / 4) — caller clamps both (neither is a full byte range the
; way PLOT's x is, so this needs the same clamp PLOT's y already
; gets).
; In:  B = cx (0-63), C = cy (0-47), A = attribute byte, D = OVER flag
;      (0 = set/OR, nonzero = XOR-toggle) — same conventions as
;      GFX_WRITE_PIXEL
; Out: none
; Destroys: AF, BC, DE, HL
; ============================================================================
GFX_CPLOT:
    ...
    ld   a, c
    and  1
    ld   (GFX_CPLOT_SCANLINE_START), a   ; 0 or 1 for now — resolved
                                        ; to a real scanline offset
                                        ; (0 or 4) below
    ld   a, b
    and  1
    ld   (GFX_CPLOT_PATTERN), a          ; 0 or 1 for now too

    ld   a, c
    srl  a                              ; char_row = cy >> 1 (cy maxes
                                        ; at 47, so a single shift —
                                        ; no wraparound risk, unlike
                                        ; the y>>3 trick elsewhere)
    ld   (GFX_PIXEL_ROW), a
    ld   a, b
    srl  a                              ; char_col = cx >> 1
    ld   (GFX_PIXEL_COL), a

    ld   a, (GFX_CPLOT_SCANLINE_START)
    or   a
    jr   z, .top
    ld   a, 4
    jr   .scanline_done
.top:
    xor  a
.scanline_done:
    ld   (GFX_CPLOT_SCANLINE_START), a

    ld   a, (GFX_CPLOT_PATTERN)
    or   a
    jr   z, .left
    ld   a, $0F
    jr   .pattern_done
.left:
    ld   a, $F0
.pattern_done:
    ld   (GFX_CPLOT_PATTERN), a

    ; bitmap address of scanline 0 of this cell — same ROW_BASE_TABLE
    ; lookup GFX_PIXEL_ADDR_SETUP already uses
    ld   a, (GFX_PIXEL_ROW)
    call GFX_ROW_BASE_ADDR
    ex   de, hl
    ld   a, (GFX_PIXEL_COL)
...
.no_col_carry:
    ...
    ld   d, a
    ld   h, d
    ld   l, e                            ; HL = bitmap address of the
                                         ; first of the 4 target
                                         ; scanlines
...
.xor_loop:
    ...
    ld   (hl), a
    ld   a, h
    add  a, 1                            ; +1 to the high byte = +256
                                         ; to the address = next
                                         ; scanline down
    ld   h, a
    djnz .xor_loop
...
.attr:
    ; Color the covering attribute — which formula depends on the
    ; active video mode, same split GFX_WRITE_PIXEL's own attribute
    ; step already has. Normal mode colors the whole 8x8 cell (real
    ; attribute-clash hardware); High Resolution Graphics mode needs
    ; one attribute write per touched scanline (4, matching this
    ; quadrant's own 8x1 resolution) rather than one whole-cell write —
    ; this was a known, documented gap when MODE first shipped (CPLOT
    ; wrote through GFX_SET_ATTR directly rather than routing through
    ; GFX_WRITE_PIXEL, so it never inherited that routine's mode-
    ; awareness for free the way PLOT/LINE/BLOCK/CIRCLE did).
    ld   a, (GFX_MODE)
    cp   1
    jr   z, .attr_ext
...
.attr_ext:
    ; real_y of the first touched scanline = row*8 + scanline_start —
    ; identical reconstruction GFX_WRITE_PIXEL's own mode-1 branch
    ; already uses and this project already Python-verified (row maxes
    ; at 23, so row*8 maxes at 184 — no overflow risk in a byte).
    ; Unrolled 4 times rather than a counted loop: GFX_SET_ATTR_EXT
    ; destroys DE, which would otherwise have to survive as this loop's
    ; own counter — unrolling sidesteps that instead of fighting it.
    ; B/C (real_y/col) genuinely do survive each call, per GFX_SET_
    ; ATTR_EXT's own documented "Destroys: AF, DE, HL" contract.
    ld   a, (GFX_PIXEL_ROW)
    add  a, a
    add  a, a
    add  a, a
    ...
    ld   a, (GFX_CPLOT_SCANLINE_START)
    add  a, b
    ld   b, a                            ; B = real_y (first scanline)
    ld   a, (GFX_PIXEL_COL)
    ld   c, a                            ; C = col

    ld   a, (GFX_PIXEL_ATTR)
    call GFX_SET_ATTR_EXT
    inc  b
    ld   a, (GFX_PIXEL_ATTR)
    call GFX_SET_ATTR_EXT
    inc  b
