...
;
; NOT PART OF THE PRODUCTION ROM BUILD (2026-08-22): the real Home ROM
; (rom/test_basic.asm) no longer INCLUDEs this file — this whole
; module moved to EXROM as rom/exrom_editor.asm in a ROM-shrink pass,
; once the string-functions feature alone left Home ROM hundreds of
; bytes over its 16K cap. basic/basic.asm now reaches the editor
; through its own BASIC_EDITOR_*_EXROM wrappers (page in, call a fixed
; EXROM entry stub at $C060-$C089, page out), not by calling labels in
; this file directly — see rom/exrom_checker.asm's own entry-stub doc
; block and rom/exrom_editor.asm's own header for the full migration.
;
; THIS FILE IS KEPT, UNCHANGED, as a standalone-testable reference
; copy — rom/test_editor.asm (CLAUDE.md's own documented kernel/editor
; test target) still builds and exercises it as a Home-only module in
; isolation, and dozens of older ad-hoc rom/test_*.asm harnesses still
; INCLUDE it directly too. rom/exrom_editor.asm is NOT auto-generated
; from this file — it's a hand-adapted copy (external calls routed
; through KTAB_* instead of called directly) that must be updated BY
; HAND alongside any real logic change made here, exactly the "two
; copies to keep in sync" risk this project's own checker_keywords.inc
; explicitly warns about for DATA tables — no automated tripwire exists
; for this CODE duplication yet. Treat any bugfix or behavior change to
; the editor's actual logic as needing BOTH files touched, and verify
; against BOTH rom/test_editor.asm (this file) and a real interactive
; EDIT session through rom/test_basic.asm + exrom.bin (rom/exrom_
; editor.asm) before considering it done.
;
...
;
; Public API used to be declared in include/kernel_api.inc — no longer,
; since this file isn't part of the production build any more (see the
; note above). BASIC (basic/) calls EDITOR_ENTER to hand control to the
; editor and gets control back at EDITOR_EXIT with the edited text
; committed to the program area via the normal kernel storage routines
; (kernel/memory, not yet written).
;
...
; ---- EDIR_* direction constants for EDITOR_MOVE_CURSOR/EDITOR_NAV_HOOK
; moved to include/sysvars.inc (2026-08-22), alongside EDITOR_NAV_HOOK
; itself, when rom/exrom_editor.asm (the production copy) needed them
; visible from a second file that doesn't define EDITOR_MOVE_CURSOR —
; see that file's own header for the full reasoning. Already reachable
; here via the INCLUDE above; no longer defined in this file directly.
...
;
; Verified against a Python reference (tools/ has no permanent copy of
; this one — it's small enough to have been checked inline) across:
; empty line, exactly-32-chars, break exactly at column 31, no-space
; 40-char run, many short words, and leading/repeated spaces. All
; offsets 0..line_length were checked to map back to a valid (row, col)
; via the same span convention EDITOR_WRAP_OFFSET_TO_ROWCOL uses below.
;
...
; before wrapping another (true today: BASIC_REDRAW_PROGRAM calls this
; once per statement/active-line, printing immediately after each call
...
EDITOR_WRAP_CALC:
    ...
    ; total content length, scanning for the null terminator — same
    ; bound as EDITOR_SCAN_LEN, but this routine can't reuse that one
    ; directly since EDITOR_SCAN_LEN is hardwired to EDIT_LINE_BUF and
    ; EDITOR_WRAP_CALC must also work on DETOK_BUF (BASIC_REDRAW_
    ; PROGRAM's settled-statement path)
    ld   b, 0
.scan_len_loop:
    ld   a, b
    cp   EDIT_LINE_BUF_LEN - 1
...
.not_space:
    ...
    ; drawn — see EDITOR_WRAP_OFFSET_TO_ROWCOL's own header for how a
    ; cursor offset landing exactly on that skipped space is handled)
    ld   a, c
...
; this shape). Promoted from what used to be a local helper private to
; EDITOR_WRAP_CALC (and two more near-identical copies duplicated
; separately in basic.asm) into one real shared routine — the
; consolidation [stated] asked for after noticing the same pattern
; recurring 11 times total across editor.asm and basic.asm combined.
; No sign extension needed — every real caller's index is 0-7.
...
; Checked against the Python reference for every offset 0..line_length
; alongside EDITOR_WRAP_CALC's own verification.
;
; In:  A = buffer offset (0-127)
